...
import { Button, Card, CardBody, CardHeader, Field, Input, Notice, RiftMark, Toggle, cn } from '@rift/ui';
      ...
      <ScreenHeader
        ...
        actions={
          <Link href={`/dashboard/sites/${siteId}/configuration`}>
            <Button variant="tonal" iconAfter="arrowRight">Back to Rift’s configuration</Button>
          </Link>
        }
      />
              ...
              <CardBody className="p-7">
                ...
                <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                  <div className="flex flex-col gap-5">
                    {/* Read-only, and marked so. The platform has no endpoint
                        that persists banner copy — these were editable inputs
                        with nothing behind them, so anything typed here was
                        discarded on navigation while looking saved. Showing the
                        generated wording is useful; pretending it can be
                        changed here is not. */}
                    <Notice tone="neutral" icon="info" title="Generated by Rift, not editable yet">
                      This wording comes from your configuration. Editing it here is not available — the
                      platform has no endpoint for banner copy — so these fields show what Rift produced
                      rather than accepting changes that would be lost.
                    </Notice>
                    <Field label="Banner title" htmlFor="banner-title">
                      <Input id="banner-title" readOnly value={config.consent.banner.title} />
                    </Field>
                    <Field
                      label="Banner body"
                      htmlFor="banner-body"
                    ...
                    >
                      <Input id="banner-body" readOnly value={config.consent.banner.body} />
                    </Field>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <Field label="Accept label" htmlFor="banner-accept">
                        <Input id="banner-accept" readOnly value={config.consent.banner.acceptAllLabel} />
                      </Field>
                      <Field label="Reject label" htmlFor="banner-reject">
                        <Input id="banner-reject" readOnly value={config.consent.banner.rejectLabel} />
                      </Field>
                    </div>
                  </div>
                ...
                <CardBody className="p-7">
                  ...
                  <dl className="mt-5">
                    ...
                    <div className="flex items-start justify-between gap-4 pt-4">
                      ...
                      {/* Disabled: this reflects platform behaviour and has no
                          write behind it. A live-looking switch that springs
                          back is a worse account than one that says so. */}
                      <Toggle checked disabled aria-label="Re-ask on material configuration change" />
                    </div>
                  </dl>
                </CardBody>
              </Card>
                    ...
                    {([
                      ...
                      <div key={label} className={`flex items-start justify-between gap-4 py-4 ${i < arr.length - 1 ? 'border-b border-md-outline-variant/40' : ''}`}>
                        <div className="min-w-0 flex-1">
                          ...
                          {locked ? (
                            <div className="mt-1.5 text-label-small font-medium uppercase tracking-[0.08em] text-md-on-surface-variant">
                              Always on
                            </div>
                          ) : null}
                        </div>
                        {/* All four are disabled, not just the locked one.
                            None has a write behind it: these describe what the
                            platform records, and a switch that moves and
                            changes nothing is a claim the product cannot
                            honour. `locked` still distinguishes the one that
                            could never be turned off even once this is
                            editable. */}
                        <Toggle checked={on} disabled aria-label={label} />
                      </div>
                    ))}
                  </div>
                </CardBody>
