...
- Since Phase 8B it has **exactly one consumer**: `api/lib/consent-config.ts`, which uses it to annotate a consent proposal an operator reads. `policy-boundary.test.ts` keeps an allowlist of that one file and separately asserts that no *route handler* imports it, so the engine can inform a person and cannot gate a request. Nothing it produces reaches the browser. See [policy-engine.md](policy-engine.md) and [consent-experience.md](consent-experience.md).
